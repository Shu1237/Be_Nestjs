...
describe('GerneService', () => {
  ...
  describe('3.findAllGernes', () => {
    it('✅ 3.1 should return paginated genres', async () => {
      const mockGernes = [
      ...
      const mockQueryBuilder = {
        ...
        skip: jest.fn().mockReturnThis(),
        take: jest.fn().mockReturnThis(),
        getManyAndCount: jest.fn().mockResolvedValue([mockGernes, 2]),
      };
      (mockGerneRepo.createQueryBuilder as jest.Mock).mockReturnValue(
        mockQueryBuilder,
      );

      const filters = { page: 1, take: 10 };
      const result = await service.findAllGernes(filters);

      expect(result.data).toEqual(mockGernes);
      expect(result.meta.total).toBe(2);
    });

    it('✅ 3.2 should apply pagination parameters', async () => {
      const mockQueryBuilder = {
        select: jest.fn().mockReturnThis(),
        where: jest.fn().mockReturnThis(),
        andWhere: jest.fn().mockReturnThis(),
      ...
      const filters = { page: 2, take: 15 } as GernePaginationDto;
      await service.findAllGernes(filters);

      expect(mockQueryBuilder.skip).toHaveBeenCalledWith(15); // (page-1) * take
      expect(mockQueryBuilder.take).toHaveBeenCalledWith(15);
    });

    it('✅ 3.3 should apply sorting parameters', async () => {
      const mockQueryBuilder = {
        select: jest.fn().mockReturnThis(),
        where: jest.fn().mockReturnThis(),
        andWhere: jest.fn().mockReturnThis(),
      ...
      } as GernePaginationDto;
      await service.findAllGernes(filters);

      expect(mockQueryBuilder.orderBy).toHaveBeenCalled();
    });

    it('✅ 3.4 should apply text search filter', async () => {
      const mockQueryBuilder = {
        select: jest.fn().mockReturnThis(),
        where: jest.fn().mockReturnThis(),
        andWhere: jest.fn().mockReturnThis(),
    ...
    it('❌ 3.5 should handle database error', async () => {
      const mockQueryBuilder = {
        select: jest.fn().mockReturnThis(),
        where: jest.fn().mockReturnThis(),
        andWhere: jest.fn().mockReturnThis(),
      ...
      const filters = { page: 1, take: 10 };
      await expect(service.findAllGernes(filters)).rejects.toThrow(
        'Database error',
      );
    });

    it('✅ 3.6 should handle missing pagination parameters', async () => {
      const mockQueryBuilder = {
        select: jest.fn().mockReturnThis(),
        where: jest.fn().mockReturnThis(),
        andWhere: jest.fn().mockReturnThis(),
      ...
      const filters = { page: 1, take: 10 } as GernePaginationDto; // Provide required properties
      const result = await service.findAllGernes(filters);
