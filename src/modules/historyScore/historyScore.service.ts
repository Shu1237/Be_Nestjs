...
export class HistoryScoreService {
    ...
    async getAllHistoryScore(filters: HistoryScorePaginationDto) {
        const qb = this.historyScoreRepository.createQueryBuilder('history_score')
        ...
        applySorting<HistoryScore>(qb, filters.sortBy, filters.sortOrder, allowedFields, 'history_score.created_at');
    ...
    async getHistoryScoreByUserId(filters: HistoryScorePaginationDto & { userId: string }) {
        const qb = this.historyScoreRepository.createQueryBuilder('history_score')
            .leftJoinAndSelect('history_score.user', 'user')
            .leftJoinAndSelect('history_score.order', 'order')
            .leftJoinAndSelect('order.promotion', 'promotion')
