...
export class OverviewService {
    ...
    async getRevenueGrowth(days: number = 30): Promise<{ currentPeriod: number; previousPeriod: number; growthPercentage: number }> {
        const currentDate = new Date();
    ...
    async getCustomerRetentionRate(): Promise<{ retentionRate: number; returningCustomers: number; totalCustomers: number }> {
    ...
    async getMoviePerformanceAnalysis(): Promise<{ totalMovies: number; activeMovies: number; averageRevenue: number }> {
