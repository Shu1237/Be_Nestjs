...
describe('CinemaRoomService', () => {
  ...
  describe('1.1.create edge cases', () => {
    ...
    it('❌ 1.1.4 should throw if save throws error', async () => {
      ...
      (mockCinemaRoomRepo.save as jest.Mock).mockRejectedValue(new Error('Save error'));
      await expect(service.create({ cinema_room_name: 'A' } as any)).rejects.toThrow('Save error');
    });
  ...
  describe('1.2.findAll', () => {
    ...
    it('✅ 1.2.6 should sort by allowed field', async () => {
      ...
      await service.findAll({ page: 1, take: 10, sortBy: 'cinema_room_name', sortOrder: 'ASC' } as any);
  ...
  describe('3.update', () => {
    ...
    it('❌ 3.3 should throw if update value is same as existing name but different ID', async () => {
      (mockCinemaRoomRepo.findOne as jest.Mock).mockResolvedValue({ id: 2, cinema_room_name: 'B' }); // another room
      await expect(service.update(1, { cinema_room_name: 'B' })).rejects.toThrow(BadRequestException);
    });
  ...
  describe('4.remove', () => {
    ...
    it('❌ 4.3 should throw error if remove fails', async () => {
      (mockCinemaRoomRepo.findOne as jest.Mock).mockResolvedValue({ id: 1 });
      (mockCinemaRoomRepo.remove as jest.Mock).mockRejectedValue(new Error('Remove failed'));
    
      await expect(service.remove(1)).rejects.toThrow('Remove failed');
    });
    it('❌ 4.4 should throw if remove fails due to DB error', async () => {
      (mockCinemaRoomRepo.findOne as jest.Mock).mockResolvedValue({ id: 1 });
      (mockCinemaRoomRepo.remove as jest.Mock).mockRejectedValue(new Error('SQL error'));
  ...
  it('✅ 7.1 should return all cinema rooms where is_deleted is false', async () => {
    ...
    expect(mockCinemaRoomRepo.find).toHaveBeenCalledWith({ where: { is_deleted: false } });
  });
  ...
  it('❌ 7.3 should throw error if repository fails', async () => {
    (mockCinemaRoomRepo.find as jest.Mock).mockRejectedValue(new Error('DB error'));
  ...
  it('❌ 7.6 should throw if find throws unexpected error', async () => {
    (mockCinemaRoomRepo.find as jest.Mock).mockRejectedValue(new Error('Unexpected'));
