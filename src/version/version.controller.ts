...
export class VersionController {
  ...
  async create(@Body() createVersionDto: CreateVersionDto, @Req() req) {
    ...
    if (user.role_id !== Role.ADMIN && user.role_id !== Role.EMPLOYEE) {
      throw new ForbiddenException('Unauthorized: Only admin can create a version.');
    }
  ...
  // @ApiResponse({ status: 403, description: 'Forbidden.' })
  // async update(@Param('id') id: number, @Body() updateVersionDto: UpdateVersionDto, @Req() req) {
  //   const user = req.user as JWTUserType;
  //   if (user.role_id !== Role.ADMIN && user.role_id !== Role.EMPLOYEE) {
  //     throw new ForbiddenException('Unauthorized: Only admin can update a version.');
  //   }
  ...
  async remove(@Param('id') id: number, @Req() req) {
    const user = req.user as JWTUserType;
    if (user.role_id !== Role.ADMIN && user.role_id !== Role.EMPLOYEE) {
      throw new ForbiddenException('Unauthorized: Only admin can delete a version.');
    }
